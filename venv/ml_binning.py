# -*- coding: utf-8 -*-
"""
Created on Tue Sep 25 13:51:16 2018
...
class Binning:
    ...
    def binning_num(self, column):
        ...
        d1_1_temp = pd.DataFrame({"X": an_df[column], "Y": an_df[self.gbflag],
                                  column: pd.qcut(an_df[column], self.bin_num, precision=2, duplicates='drop')})
        d1_1 = d1_1_temp.sort_values(by=['X'])
        if nan_df.shape[0] > 0:
            d1_2 = pd.DataFrame(
                {"X": nan_df[column], "Y": nan_df[self.gbflag], column: pd.qcut(nan_df[column], 1, duplicates='drop')})
            d1 = pd.concat([d1_1, d1_2])
        else:
            d1 = d1_1
        d2 = pd.crosstab(np.array(d1[column]), np.array(d1['Y']), margins=True)
        ...
        d2.columns.name = column
        d2.index.name = '{}_interval'.format(column)
        return d2

    # 若是char型或者是 pd.unique(test_df['X_LTV_GENDER']).shape[0] 小于50的
    ...
    def binning_print(self):
        ...
        for column in self.bin_df.columns:
            column_df = pd.DataFrame([[column, column, column, column, column]],
                                     columns=[0, 1, 'All', 'Badrate', 'Pctn'], index=[column])
            if column != self.gbflag and self.bin_df[column].unique().shape[0] < 50:
                if pd.unique(self.bin_df[column]).shape[0] < 20 or self.bin_df[column].dtype not in ['float', 'int',
                                                                                                     'float64',
                                                                                                     'int64']:
    ...
    def iv_column(self, column):
        ...
        if pd.unique(self.bin_df[column]).shape[0] > 1 and re.sub(u'_', '', column.lower()) not in [self.gbflag,
                                                                                                    'opendate', 'appid',
                                                                                                    'userid']:
            if pd.unique(self.bin_df[column]).shape[0] < 20 or self.bin_df[column].dtype not in ['float', 'int',
                                                                                                 'float64', 'int64']:
            ...
            d2 = d2.assign(
                woe=d2.apply(lambda d2: math.log((d2['Goodratio'] + 0.000000001) / (d2['Badratio'] + 0.000000001)),
                             axis=1))
